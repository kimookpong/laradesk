<template>
    <div :class="isVideo ? 'col-span-full border w-full mb-1 mr-1 overflow-hidden' : 'flex items-center border w-full h-12 py-3 px-2 mb-1 mr-1'">
        <template v-if="isVideo">
            <video class="w-full block" style="max-height: 360px;" controls preload="metadata">
                <source :src="videoSrc" :type="details.mime">
            </video>
            <div class="flex items-center py-2 px-2">
                <div class="mr-2">
                    <svg-vue v-if="details.extension === 'mp4'" class="h-5 w-5" icon="file-extension.mp4"/>
                ...
                <button v-if="downloadButton" class="mr-2 flex-shrink-0" type="button" @click="download">
                    <svg-vue class="h-5 w-5 text-gray-800" icon="font-awesome.arrow-to-bottom-light"></svg-vue>
                </button>
                <button v-if="removeButton" class="flex-shrink-0" type="button" @click="$emit('remove')">
...
export default {
    ...
    computed: {
        isVideo() {
            const videoExtensions = ['mp4', 'webm', 'mov', 'avi', 'mkv', 'ogv', 'ogg', 'm4v', '3gp'];
            return videoExtensions.includes(this.details.extension);
        },
        videoSrc() {
            if (this.details.url) {
                return this.details.url;
            }
            return this.details.download + '?token=' + localStorage.getItem('token');
        }
    },
    methods: {
        download() {
